...
const ProductsPanel: React.FC = () => {
  ...
  const open = (p?: Prod) => {
    ...
    // نحافظ على جميع حقول metadata الأصلية (مثل created_by) ونضيف أو نضمن الحقول الأساسية
    base.metadata = {
      ...md,
  ...
  const save = async () => {
    ...
    // تحويل tags من نص إلى مصفوفة نظيفة
    const tagsArray = typeof editing.tags === 'string'
    ...
    await cms('cms_product_save', { product: productToSave });
    setBusy(false);
    setEditing(null);
    load();
  };
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            className="w-full border border-[#e6e6e6] rounded-lg pl-9 pr-3 py-2.5 text-sm focus:border-[#FF6A00] outline-none"
          />
        </div>
        <button
          onClick={() => open()}
          className="bg-[#FF6A00] text-white px-5 py-2.5 rounded-lg flex items-center gap-1.5 text-sm font-medium hover:bg-[#e85f00] shadow-sm"
        >
          ...
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
              statusFilter === key
                ...
                : 'border-[#e6e6e6] text-[#666] hover:border-[#1D1D1D]'
            }`}
          >
            {label}
          </button>
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
        {filtered.length === 0 && (
          <p className="text-[#8D8D8D] col-span-full py-10 text-center">No products yet.</p>
        )}
        {filtered.map((p) => (
          <div
            key={p.id}
            className="group bg-white border border-[#eee] rounded-xl overflow-hidden hover:shadow-md transition-shadow"
          >
            <div className="relative aspect-[5/4] bg-[#f6f6f6]">
              {p.images?.[0] ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <ImageIcon size={22} className="text-[#ddd]" />
                </div>
              )}
              <span
              ...
              <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => open(p)} title="Edit" className="bg-white/95 rounded-md p-1.5 shadow hover:text-[#FF6A00]">
                  <Pencil size={14} />
                </button>
                <button onClick={() => dup(p.id)} title="Duplicate" className="bg-white/95 rounded-md p-1.5 shadow">
                  <Copy size={14} />
                </button>
                <button
                  onClick={() => archive(p)}
                  title={p.status === 'archived' ? 'Restore from archive' : 'Archive'}
                  className={`bg-white/95 rounded-md p-1.5 shadow ${
                    p.status === 'archived' ? 'hover:text-emerald-600' : 'hover:text-amber-600'
                  }`}
                >
                ...
                {owner && (
                  <button
                    ...
                    className="bg-white/95 rounded-md p-1.5 shadow hover:text-red-500"
                  >
                    <Trash2 size={14} />
                  </button>
                )}
            ...
            <div className="p-3">
              ...
              <p className="text-[11px] text-[#aaa] truncate mt-0.5">
                {(p.metadata?.categories || []).map(catName).filter(Boolean).join(', ') ||
                  p.product_type ||
                  'Uncategorized'}
              </p>
              {p.metadata?.created_by?.name && (
                <span
                  ...
                  style={{
                    background: `${p.metadata.created_by.color || '#888'}1a`,
                    color: p.metadata.created_by.color || '#666',
                    border: `1px solid ${p.metadata.created_by.color || '#ccc'}55`,
                  }}
      ...
      {editing && (
        ...
        >
          <div
            className="bg-white w-full max-w-3xl max-h-[94vh] overflow-auto rounded-2xl"
          ...
          >
            <div className="sticky top-0 bg-white/95 backdrop-blur border-b border-[#eee] px-6 py-4 flex items-center justify-between z-10">
              ...
              <button onClick={() => setEditing(null)} className="text-[#999] hover:text-black">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-7">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input
                  ...
                  className="border border-[#e6e6e6] rounded-lg px-3 py-2.5 col-span-2 focus:border-[#FF6A00] outline-none"
                />
                <input
                  value={editing.product_type || ''}
                  onChange={(e) => setEditing({ ...editing, product_type: e.target.value })}
                  placeholder="Type (e.g. Poster)"
                  className="border border-[#e6e6e6] rounded-lg px-3 py-2.5 focus:border-[#FF6A00] outline-none"
                />
                <input
                ...
                <input
                  ...
                  placeholder="Base price (MAD)"
                  className="border border-[#e6e6e6] rounded-lg px-3 py-2.5 focus:border-[#FF6A00] outline-none"
                />
                <select
                  value={editing.status}
                  onChange={(e) => setEditing({ ...editing, status: e.target.value })}
                  className="border border-[#e6e6e6] rounded-lg px-3 py-2.5"
                >
                ...
                <textarea
                  ...
                  className="border border-[#e6e6e6] rounded-lg px-3 py-2.5 col-span-2 h-20 focus:border-[#FF6A00] outline-none"
                />
              </div>
              ...
              <section>
                <div className="flex items-center gap-2 mb-3">
                  <Tag size={15} className="text-[#FF6A00]" />
                  <h4 className="font-medium text-sm">
                    Categories <span className="text-[#aaa] font-normal">(select one or more)</span>
                  </h4>
                </div>
                {tops.length === 0 ? (
                  <p className="text-xs text-[#aaa]">No categories yet — create them in the Categories tab.</p>
                ) : (
                    ...
                    {tops.map((t) => {
                      ...
                      return (
                        <div key={t.id} className="border border-[#f0f0f0] rounded-lg p-3">
                          ...
                          >
                            {sel.includes(t.id) && <Check size={12} />}
                            {t.title}
                          </button>
                          {kids.length > 0 && (
                            <div className="flex flex-wrap gap-1.5 mt-2 pl-2">
                              {kids.map((k) => (
                                <button
                                  key={k.id}
                                  type="button"
                                  onClick={() => toggleCat(k.id)}
                                  className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] ${
                                    sel.includes(k.id)
                                      ? 'bg-[#1D1D1D] text-white'
                                      : 'bg-[#fafafa] border border-[#eee] text-[#777] hover:border-[#FF6A00]'
                                  }`}
                                >
                                  {sel.includes(k.id) && <Check size={10} />}
                                  {k.title}
                                </button>
                              ))}
                            </div>
              ...
              <section>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <ImageIcon size={15} className="text-[#FF6A00]" />
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="text-xs inline-flex items-center gap-1 border border-[#e6e6e6] rounded-lg px-3 py-1.5 hover:border-[#FF6A00]"
                    >
                      <FolderOpen size={13} /> From media
                    </button>
                ...
                <div className="flex flex-wrap gap-2">
                  {editing.images?.map((img, i) => (
                    <div key={i} className="relative group">
                      <img
                        src={img}
                        className="w-20 h-20 object-cover rounded-lg border border-[#eee]"
                        alt=""
                      />
                      {i === 0 && (
                        <span className="absolute bottom-1 left-1 bg-[#FF6A00] text-white text-[9px] px-1.5 rounded">
                          Cover
                        </span>
                      )}
                      <button
                        onClick={() => removeImage(i)}
                        className="absolute -top-2 -right-2 bg-white border rounded-full p-0.5 shadow opacity-0 group-hover:opacity-100"
                      >
                        <X size={12} />
                      </button>
                    </div>
                  ...
                  {(!editing.images || editing.images.length === 0) && (
                    <p className="text-xs text-[#bbb] py-6">No images selected yet.</p>
                  )}
                </div>
              </section>
              ...
              <section>
                <div className="flex items-center gap-2 mb-3">
                  <Ruler size={15} className="text-[#FF6A00]" />
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium border ${
                          active
                            ...
                            : 'border-[#e6e6e6] text-[#666] hover:border-[#FF6A00]'
                        }`}
                      >
                        {s}
                      </button>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="Custom size e.g. 120x180"
                    className="border border-[#e6e6e6] rounded-lg px-3 py-2 text-sm flex-1 focus:border-[#FF6A00] outline-none"
                  />
                  <button
                    type="button"
                    onClick={addCustomSize}
                    className="bg-[#1D1D1D] text-white px-4 rounded-lg text-sm"
                  >
                    Add
                  </button>
                </div>
              ...
              <section>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-medium text-sm">
                    Add-ons & Extras{' '}
                    <span className="text-[#aaa] font-normal">(toggle + set extra price)</span>
                  </h4>
                </div>
                  ...
                  {(editing.metadata.addons || []).map((a: AddOn, i: number) => (
                    <div
                      ...
                      className={`flex items-center gap-3 border rounded-lg px-3 py-2 ${
                        a.enabled ? 'border-[#FF6A00] bg-[#FFF6F0]' : 'border-[#eee]'
                      }`}
                    >
                      <button
                        type="button"
                      ...
                      <div className="flex items-center gap-1 text-sm">
                        ...
                        <input
                          type="number"
                          value={a.price ? a.price / 100 : ''}
                          onChange={(e) =>
                            setAddonPrice(i, Math.round(Number(e.target.value) * 100))
                          }
                          className="w-20 border border-[#e6e6e6] rounded px-2 py-1 text-right text-sm"
                        />
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <input
                    ...
                    placeholder="MAD"
                    className="border border-[#e6e6e6] rounded-lg px-3 py-2 text-sm w-24 focus:border-[#FF6A00] outline-none"
                  />
                  <button
                    type="button"
                    onClick={addAddon}
                    className="bg-[#FF6A00] text-white px-4 rounded-lg text-sm flex items-center gap-1"
                  >
                    <Plus size={14} /> Add
                  </button>
                </div>
              ...
              <section>
                <div className="flex items-center gap-2 mb-3">
                  <Layers size={15} className="text-[#FF6A00]" />
                  <h4 className="font-medium text-sm">
                    Variants — Material × Size{' '}
                    <span className="text-[#aaa] font-normal">(optional, priced separately)</span>
                  </h4>
                </div>
                <div className="space-y-2">
                  {MATERIALS.map((m) => (
                      ...
                      {PRESET_SIZES.map((s) => {
                        ...
                        return (
                          <button
                            key={s}
                            type="button"
                            onClick={() => toggleVariant(m, s)}
                            className={`px-2 py-1 text-[11px] rounded border ${
                              active
                                ? 'bg-[#1D1D1D] text-white border-[#1D1D1D]'
                                : 'border-[#e6e6e6] text-[#888] hover:border-[#FF6A00]'
                            }`}
                          >
                            {s}
                          </button>
                ...
                {editing.variants!.length > 0 && (
                  <div className="mt-3 space-y-1">
                    {editing.variants!.map((v) => (
                      <div
                        key={v.title}
                        className="flex items-center justify-between text-sm bg-[#fafafa] rounded px-2 py-1"
                      >
                        <span>{v.title}</span>
                        <input
                          type="number"
                          ...
                          onChange={(e) =>
                            setEditing({
                              ...editing,
                              variants: editing.variants!.map((x) =>
                                x.title === v.title
            ...
            <div className="sticky bottom-0 bg-white border-t border-[#eee] px-6 py-4 flex justify-end gap-3">
              <button
                onClick={() => setEditing(null)}
                className="px-4 py-2 text-sm text-[#8D8D8D]"
              >
                Cancel
              </button>
              <button
                disabled={busy}
                onClick={save}
                className="bg-[#FF6A00] text-white px-6 py-2.5 rounded-lg text-sm font-medium disabled:opacity-50 hover:bg-[#e85f00]"
              >
      ...
      {picker && editing && (
        ...
        >
          <div
            className="bg-white w-full max-w-3xl max-h-[85vh] overflow-auto rounded-2xl p-6"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            {media.length === 0 ? (
              <p className="text-sm text-[#aaa] py-10 text-center">No media uploaded yet.</p>
            ) : (
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                {media.map((m) => {
                  ...
                  return (
                    <button
                      key={m.id}
                      onClick={() => toggleFromMedia(m.url)}
                      className={`relative aspect-square rounded-lg overflow-hidden border-2 ${
                        sel ? 'border-[#FF6A00]' : 'border-transparent'
                      }`}
                    >
                      <img src={m.url} className="w-full h-full object-cover" alt="" />
                      {sel && (
                        <span className="absolute inset-0 bg-[#FF6A00]/25 flex items-center justify-center">
            ...
            <div className="flex justify-end mt-5">
              <button
                onClick={() => setPicker(false)}
                className="bg-[#FF6A00] text-white px-6 py-2.5 rounded-lg text-sm font-medium"
              >
                Done ({(editing.images || []).length} selected)
              </button>
            </div>
          </div>
        </div>
